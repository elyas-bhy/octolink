...
public class OctolinkOverlapProcessor implements OverlapProcessor {
	...
	public OctolinkOverlapProcessor() {
	...
	private void computeOneOverlap(Overlappable overlappable,
		...
		for (Overlappable targetOverlappable : movablesTmp) {
			if (targetOverlappable != overlappable) {
				Shape targetShape;
				targetShape = OctolinkIntersectTools.getIntersectShape(
	...
	private Shape intersectionComputation(Overlappable overlappable) {
		if (overlappable instanceof Movable) {
			...
			return OctolinkIntersectTools.getIntersectShape(movable,
					oppositeSpeedVector);
		} else {
